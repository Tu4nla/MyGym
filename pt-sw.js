...
// V13.4 intentionally does not intercept fetch. HTML/JS/CSS always use the network/browser cache rules.
...
self.addEventListener('push',event=>{
  ...
  const options={body:data.body||'Bạn có một việc cần xử lý.',tag:data.tag||'mygym-assistant',renotify:false,data:{url:data.url||'./pt.html?v=134#assistant',taskId:data.taskId||null,phase:data.phase||null},silent:false};
...
self.addEventListener('notificationclick',event=>{
  ...
  event.waitUntil((async()=>{
    ...
    const target=new URL(event.notification.data?.url||'./pt.html?v=134#assistant',self.registration.scope).href;
