...
LESSON_PATH = DATA / "lessons" / "c" / "c10.json"
PAYLOAD_PATH = ROOT / "scripts" / "c10_payload.txt"
...
(ROOT / "validation-c10.json").write_text(json.dumps(metrics, ensure_ascii=False, indent=2) + "\n")
print(json.dumps(metrics, ensure_ascii=False, indent=2))
assert lesson["id"] == "c10"
...
entry = next(i for i in chapter["lessons"] if i["id"] == "c10")
...
assert plan["current"] in ("c10", "c11")
if "c10" not in plan["completed"]: plan["completed"].append("c10")
plan["current"] = "c11"
plan_path.write_text(json.dumps(plan, ensure_ascii=False, indent=2) + "\n")

index_path = DATA / "search-index.json"
index = json.loads(index_path.read_text())
if not any(i["lessonId"] == "c10" for i in index):
    index.append({
        "lessonId":"c10","code":"C10","title":"UI State và One-time Event",
        "keywords":["UI state","one-time event","StateFlow","SharedFlow","Channel","effect","command","reducer","acknowledgement","durability","idempotency","process death"],
        "headings":["State và occurrence","Delivery contract","Reducer","Lifecycle restart","SharedFlow và Channel","Acknowledgement","Durability","Process death"]
    })
index_path.write_text(json.dumps(index, ensure_ascii=False, indent=2) + "\n")
for p in (catalog_path, plan_path, index_path, LESSON_PATH): json.loads(p.read_text())
for ch in catalog["chapters"]:
    for item in ch["lessons"]:
        if item["status"] == "published":
            assert (ROOT / "android-learning" / item["path"]).exists(), item["path"]
print("C10 deep publication validation passed")
